...
class project
{
    ...
    function insert($conn,$table,$fname,$contact,$nid,$email,$password,$gender,$Address)
    {
       
        $ins=$conn->query("insert into $table VALUES('$fname','$contact','$nid','$email','$password','$gender','$Address')");
        return $ins;
    }
    function add ($conn,$table,$fname,$contact,$serial,$time,$payment,$Problem)
    {
      
       $iner = $conn->query("INSERT INTO $table VALUES('$fname ', '$contact ',' $serial',' $time',' $payment','$Problem')");
        return $iner;
    }
